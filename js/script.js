...
function reload() {
  ...
  for (let i = 1; i < 6; i++) {
    ...
    for (let k = 1; k < 101; k++) {
      ...
      for (let j = 0; j <= order.places.length - 1; j++) {
        let takenPlace = order.places[j];

        if (`A${i}` === takenPlace.sectore && k === takenPlace.place) {
      ...
      place.onclick = () => {
        ...
        let myOrder = {
          ...
          place: k,
          row:
            row.toString().length === 3
              ? row.toString().slice(0, 2)
              : row.toString()[0],
        };

        let finded = order.places.find(
          (el) => el.place === k && el.sectore === `A${i}`
        );
...
function cardCreate() {
  ...
  for (let item of order.places) {
    ...
    card.onclick = () => {
        order.places = order.places.filter(el => el.id !== item.id)
        reload()
        cardCreate();
    };
